...
**Goal: you should be able to use `R` to scrape an HTML table from the web after this lecture**
...
-   **CSS** (Cascading Style Sheets) defines the appearance of HTML elements (i.e. whether the web page is pretty or ugly)

    -   CSS selectors are patterns used to select the elements to be style
    
    -   **CSS selectors can be used to extract elements from a web page**
...
- Reading in a website

- Data organization and cleaning (most of the process)

  - Extracting website elements (e.g., tables, links, etc.)

  - Common data manipulation tasks (e.g., with `dplyr`)

  - Working with strings (e.g, `stringr`, `stringi`) and regular expressions (regex)

- Generalization: writing functions (and developing packages)
...
-   We use `read_html()` to read in the HTML page based on a specified web page's URL
...
2.  The **Developer Tools** will open in your browser. Pay attention to the **Elements** pane (*Chrome* or *Safari*) or **Inspector** pane (*Firefox*)
    ...
    *   The HTML element corresponding to the web page area (close to the table) mentioned in Step 1 is highlighted

    *   Hovering over different HTML elements will highlight different parts of the web page
...
-   This completes our scraping pipeline.
...
-   This outputs a list of all the tables existed on the web page
...
*   Notice that the table on the web page also contains team logos, URLs, etc.
...
*   Within <a>, we can grab the `href` attribute with `html_attr` 
    ...
    *   `href` specifies the URL/page associated with the link
...
*   The function `str_subset()` comes in handy here
    ...
    *   `str_subset()` returns only the vector elements that matches a specified pattern
...
*   Example 3 Wimbledon Women's singles
...
*   Then, we can retrieve text from this element with `html_text2()`
    ...
    *   There's also an `html_text()` function. Look up the documentation `?html_text` to see the differences. (Honestly, if you don't remember the differences, just try both and see which one is suitable. Ditto for `html_element()` and `html_elements()`.)
...
*   As a final step, you can try to turn the vector into a table (as a column), then clean up and create 3 columns: seed, player, and result.
...
Web APIs provides interactions between a client device and a web server using the Hypertext Transfer Protocol (HTTP) 
...
*   Fortunately, there exists many `R` packages (sports on non-sports) that provides access to APIs for obtaining data
...
### Example 4: Formula One API

(Inspiration: [Tidy Tuesday 2021-09-07](https://github.com/rfordatascience/tidytuesday/tree/master/data/2021/2021-09-07))
...
-   Web scraping case study from [Data science ethics chapter of MDSR](https://mdsr-book.github.io/mdsr2e/ch-ethics.html)

-   [Good pratice chapter from Web Scraping using R](https://jakobtures.github.io/web-scraping/goodpractice.html)
...
-   We use the same Wimbledon example as before to demonstrate `polite`

-   First, pass the URL into `bow()` to get a "session" object

    -   This gives information about the `robots.txt` and whether the web page is scrapable
...
-   Now we can use `scrape()` to get the data from the session previously created by `bow()`
...
-   The remaining steps are similar as before. We can copy the earlier code for specifying the HTML element, retrieving text, and converting to tibble.
...
-   [Web scraping chapter](https://r4ds.hadley.nz/webscraping.html) from R4DS (2e, draft)
